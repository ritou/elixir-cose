defmodule COSE.CBORTest do
  ...
  test "encode" do
    ...
    assert CBOR.encode(1_000_000, to_hex: true) == "1a000f4240"
    assert CBOR.encode(1_000_000_000_000, to_hex: true) == "1b000000e8d4a51000"
    assert CBOR.encode(18_446_744_073_709_551_615, to_hex: true) == "1bffffffffffffffff"
    assert CBOR.encode(18_446_744_073_709_551_616, to_hex: true) == "c249010000000000000000"

    assert CBOR.encode(-18_446_744_073_709_551_616, to_hex: true) == "3bffffffffffffffff"
    assert CBOR.encode(-18_446_744_073_709_551_617, to_hex: true) == "c349010000000000000000"
    ...
    assert CBOR.encode({:timetext, {:text, "2013-03-21T20:04:00Z"}}, to_hex: true) ==
             "c074323031332d30332d32315432303a30343a30305a"

    assert CBOR.encode({:timeepoch, 1_363_896_240}, to_hex: true) == "c11a514b67b0"
    assert CBOR.encode({:timeepoch, 1_363_896_240.5}, to_hex: true) == "c1fb41d452d9ec200000"
    assert CBOR.encode({:tag, 23, <<1, 2, 3, 4>>}, to_hex: true) == "d74401020304"

    assert CBOR.encode({:tag, 24, "6449455446" |> Base.decode16!()}, to_hex: true) ==
             "d818456449455446"

    assert CBOR.encode({:tag, 32, {:text, "http://www.example.com"}}, to_hex: true) ==
    ...
    assert CBOR.encode(%{{:text, "a"} => 1, {:text, "b"} => [2, 3]}, to_hex: true) ==
             "a26161016162820203"

    assert CBOR.encode([{:text, "a"}, %{{:text, "b"} => {:text, "c"}}], to_hex: true) ==
             "826161a161626163"
